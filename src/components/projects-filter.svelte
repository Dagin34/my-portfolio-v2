...
>
    ...
    <div class="relative flex-1 w-full">
        <Search
            class="absolute left-3 top-1/2 -translate-y-1/2 text-gray-400 opacity-40"
            size={16}
        />
        <input
            type="text"
    ...
    <div class="flex w-full gap-3 items-end flex-1 max-w-2xl">
        ...
        <div class="relative min-w-40">
            ...
            <div
                class="pointer-events-none absolute inset-y-0 right-0 flex items-center px-3 text-gray-400 opacity-40"
            >
                <svg
        ...
        <!-- Layout Switcher -->
        <div
            class="flex items-center gap-1 border border-border-color p-0.5 self-end md:self-auto"
        >
            <button
                onclick={() => (layout = "grid")}
                class="p-2 transition-colors {layout === 'grid'
            ...
            <button
                onclick={() => (layout = "list")}
                class="p-2 transition-colors {layout === 'list'
                    ? 'text-brand-primary bg-white/5'
                    : 'text-gray-400 opacity-60 hover:opacity-100'}"
