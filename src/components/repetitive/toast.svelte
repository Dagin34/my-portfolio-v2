    ...
    <div class="fixed top-8 left-0 right-0 flex justify-center z-10001 pointer-events-none px-4">
        ...
        >
            ...
            <div class="relative z-10 flex items-center gap-3">
                {#if type === "primary"}
                    <span
                        class="text-success font-mono text-xs select-none animate-pulse"
                        ><MessageCircleCheck size={18} /></span
                    >
                {/if}
                <span
                    class="text-white text-xs capitalize font-light tracking-[0.18em] whitespace-nowrap"
                >
                    {formattedMessage}
                </span>
            </div>
            ...
            <div
                class="absolute inset-0 bg-[linear-gradient(110deg,transparent_0%,transparent_42%,transparent_42%,rgba(29,185,84,0.35)_50%,transparent_58%,transparent_100%)] -translate-x-full animate-[shimmer_4s_infinite]"
            ></div>
        </div>
    </div>
{/if}
